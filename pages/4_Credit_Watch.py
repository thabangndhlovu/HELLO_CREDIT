...
from hellocredit.helpers import (
    MAPPED_RATINGS,
    MAPPED_RATINGS_DICT,
    COLOR_MAPPING,
...
def main():
    ...
    financial_policy_rating, financial_policy_score = percentage_to_rating(
    ...
    credit_score = (
        output_dict["calculator_output"]["credit_score"]
        + financial_policy_percentage_weighted
    )
    ...
    with col_2.container(border=True, height=150):
        ...
        sub_col_1.markdown(
            f'<h1 style="color:{"#052D3A"}">{credit_rating}</h1>',
            unsafe_allow_html=True,
        )
        sub_col_2.markdown("###### Credit Score")
        sub_col_2.markdown(
            f'<h1 style="color:{"#052D3A"}">{credit_score:.2f}</h1>',
            unsafe_allow_html=True,
        )
        st.markdown(
            f"""<div style="background-color: {COLOR_MAPPING.get(credit_rating)}; height: {8}px;"></div>""",
            unsafe_allow_html=True,
        )
    ...
    with tab_1:
        ...
        for i, (category, metrics) in enumerate(company_expected_metrics.items()):
            with cols[i]:
                st.markdown(
                    f"#### {category.replace('_', ' ').replace('metrics', '').title()}",
                    unsafe_allow_html=True,
                )
    ...
    with tab_2:
        ...
        st.markdown(f"**{rating_meta['meaning'].replace(' - ', '** - ')}")
    ...
    with tab_4:
        ...
        bayesian_ratings = [
            bayesian_output[x]["credit_rating"] for x in bayesian_output
        ]
        ...
        fig = go.Figure(
            data=[
                go.Scatter(
                    ...
                    text=[
                        f"{rating}<br>({value})"
                        for rating, value in zip(periods_ratings, actual_values)
                    ]
                    + [f"{periods_ratings[-1]}<br>({actual_values[-1]})"],
                    textposition="top center",
                    textfont=dict(size=12),
                    marker=dict(
                        color=["#052D3A"] * prediction_start_index
                        + ["#4CAF50"] * (len(actual_values) - prediction_start_index),
                        size=[15] * prediction_start_index
                        + [17] * (len(actual_values) - prediction_start_index),
                    ),
                )
            ]
        )
            ...
            for i, (label, key, kwargs) in enumerate(prob_inputs):
                with col1 if i % 2 == 0 else col2:
                    input_dict["probabilistic_model"][key] = st.number_input(
                        label, value=prob_model[key], key=key, **kwargs
                    )
    ...
    with tab_5:
        ...
        with tab_1:
            ...
            fig = go.Figure()
            for category, original_value, normalized_value, rating in zip(
                categories, values, normalized_values, ratings
            ):
                category = (
                    category.replace("_", " ").replace("metrics", "").title().strip()
                )
                fig.add_trace(
                    go.Bar(
                        y=[""],
            ...
            normalised_weights = {
                metric: (abs(weight) / total_weight)
        ...
        with tab_2:
            clean_string = (
                lambda s: s.replace("_", " ").title().replace("Metrics", "").strip()
            )

            summary_table_df = pd.DataFrame.from_dict(
                output_dict["calculator_output"]["metrics"], orient="index"
            )
            ...
            summary_table_df["Category"] = summary_table_df["Category"].apply(
                clean_string
            )
