...
def test_metrics_structure123():
    # Validate top-level structure
    assert isinstance(metrics, dict), "Metrics should be a dictionary"
    assert sum(metric['class_weight'] for metric in metrics.values()) == 1.0, "Sum of class_weights must sum to 1.0"

    for category, data in metrics.items():
        # Check for required keys and types
        required_keys = ['class_weight', 'weights', 'metrics']
        for key in required_keys:
            assert key in data, f"{category} missing '{key}'"
        ...
        # Check for required metric keys and types
        for metric, metric_data in data['metrics'].items():
            metric_required_keys = ['lower_is_better', 'thresholds']
            for key in metric_required_keys:
                assert key in metric_data, f"{metric} missing '{key}'"
                
                if key == 'lower_is_better':
                    assert isinstance(metric_data[key], bool), f"{metric} '{key}' should be bool"
                elif key == 'thresholds':
                    assert isinstance(metric_data[key], list), f"{metric} '{key}' should be list"
                    assert len(metric_data[key]) == 9, f"{metric} '{key}' len should be 9"
                    for threshold in metric_data['thresholds']:
                        assert len(threshold) == 2, f"{metric} threshold should have two elements (lower, higher)"
                        assert all(isinstance(n, (float, int)) or n is float('inf') for n in threshold), \
                            f"{metric} threshold elements should be numeric or infinity"
