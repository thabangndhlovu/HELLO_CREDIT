...
class HelloCredit:
    ...
    def __post_init__(self):
        ...
        self.compute_rating = partial(
            calculate_credit_rating, metrics=self.input_dict["configuration"]
        )
    ...
    def run_function(self) -> bool:
        ...
        # Bayesian Model
        model_output = bayesian_ridge_model(
            self.nested_dict, **self.input_dict["probabilistic_model"]
        )
...
def calculate_credit_rating(metrics, ratios):
    ...
    def determine_credit_rating(score):
        for rating, threshold in MAPPED_RATINGS:
            if score <= threshold:
                return rating
        return (
            len(MAPPED_RATINGS) // 2
        )  # Return the middle rating if no threshold is met
    ...
    scores = {
        category: calculate_category_score(category, category_data)
    ...
    total_weight = sum(
        category_data["class_weight"] for category_data in metrics.values()
    )
...
def get_expected_metrics(data, n=100):
    return {
        category: {
            metric: sum(values[-n:]) / min(len(values), n)
    ...
    for metric_group, values_dict in metrics.items():
        ...
        for metric, values in values_dict.items():
            ...
            next_periods = np.arange(
                look_back_periods, look_back_periods + periods
            ).reshape(-1, 1)
            predictions[metric_group][metric] = (
                model.predict(next_periods).tolist()
                if periods > 0
                else list(recent_values)
            )
    return predictions
